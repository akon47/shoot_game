class PlayerClass {
  ...
  setSpawnProtection(durationMs) {
    this.spawnProtectedUntil =
      performance.now() + (durationMs ? durationMs : 0);
  }
              ...
              if (hitBox) {
                if (offsetX !== 0) {
                  if (
                    isCollision(
                      {
                        left: playerHitBox.x + offsetX,
                        top: playerHitBox.y,
                        right: playerHitBox.x + playerHitBox.width + offsetX,
                        bottom: playerHitBox.y + playerHitBox.height,
                      },
                      hitBox,
                    )
                  ) {
                    offsetX =
                      offsetX > 0
                        ? hitBox.left - playerHitBox.right
                        : hitBox.right - playerHitBox.left;
                  }
                }
                if (offsetY !== 0) {
                  if (
                    isCollision(
                      {
                        left: playerHitBox.x,
                        top: playerHitBox.y + offsetY,
                        right: playerHitBox.x + playerHitBox.width,
                        bottom: playerHitBox.y + playerHitBox.height + offsetY,
                      },
                      hitBox,
                    )
                  ) {
                    offsetY =
                      offsetY > 0
                        ? hitBox.top - playerHitBox.bottom
                        : hitBox.bottom - playerHitBox.top;
                  }
                }
              }
            }
  ...
  // presetTargets: 원격 플레이어의 사격을 그대로 재현할 때 사용 (서버가 중계한 탄도 좌표)
  shoot(targetPoint, noDeviation, presetTargets) {
    if (this.weapon === "flashlight" || this.weapon === "knife") {
      ...
      switch (this.weapon) {
        ...
        case "shotgun":
          ...
          pelletCount = 7;
          pelletSpreadAngle = (Math.PI / 180) * 12;
          break;
      }
      ...
      if (presetTargets && presetTargets.length > 0) {
        ...
        bulletAngle =
          (Math.atan2(
            targetPoint.y - muzzlePoint.y,
            targetPoint.x - muzzlePoint.x,
          ) *
            180) /
          Math.PI;
      } else {
        var bulletRadian = 0.0;
        if (targetPoint !== undefined) {
          bulletRadian =
            Math.atan2(
              targetPoint.y - muzzlePoint.y,
              targetPoint.x - muzzlePoint.x,
            ) + (noDeviation ? 0 : deviationAngle);
          ...
          targetPoint.x =
            Math.cos(targetAngle) * targetDistance + this.getCenterX();
          targetPoint.y =
            Math.sin(targetAngle) * targetDistance + this.getCenterY();

          bulletAngle = (targetAngle * 180) / Math.PI;
          bulletRadian = targetAngle;
        }
        ...
        if (pelletCount > 1) {
          ...
          for (let i = 0; i < pelletCount; i++) {
            const pelletRadian =
              bulletRadian + (Math.random() - 0.5) * pelletSpreadAngle;
            targets.push({
              x: muzzlePoint.x + Math.cos(pelletRadian) * shootRange,
              y: muzzlePoint.y + Math.sin(pelletRadian) * shootRange,
            });
          }
          targetPoint = targets[0];
        } else {
      ...
      this.shootInfo = {
        ...
        targets: targets,
        angle: bulletAngle,
        hitObjectIntersection: undefined,
        hitObjectIntersections: new Array(targets.length),
      };
      ...
      if (this.onshoot) {
        this.onshoot(this, this.weapon, muzzlePoint, targets, bulletAngle);
      }
    }
  }
