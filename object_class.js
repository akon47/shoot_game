      ...
      if (obj) {
        ...
        if (rangeBox) {
          for (let j = 0; j < objectSegments.length; j++) {
            if (
              (rangeBox.left <= objectSegments[j].a.x &&
                rangeBox.right >= objectSegments[j].a.x &&
                rangeBox.top <= objectSegments[j].a.y &&
                rangeBox.bottom >= objectSegments[j].a.y) ||
              (rangeBox.left <= objectSegments[j].b.x &&
                rangeBox.right >= objectSegments[j].b.x &&
                rangeBox.top <= objectSegments[j].b.y &&
                rangeBox.bottom >= objectSegments[j].b.y)
            ) {
...
class WallObjectClass {
  ...
  getSegments(cameraClass) {
    const result = [
      ...
      {
        a: { x: this.right, y: this.top },
        b: { x: this.right, y: this.bottom },
      },
      {
        a: { x: this.left, y: this.bottom },
        b: { x: this.right, y: this.bottom },
      },
