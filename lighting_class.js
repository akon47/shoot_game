...
class SightEffectClass {
    constructor(screenWidth, screenHeight) {
        ...
        this.sightDrawingContext = this.sightCanvas.getContext("2d", { alpha: false });
    }
    ...
    drawSightLighting(drawingContext) {
        if (!debugClass.debugGraphicsVisible) {
            ...
            this.drawDebugSightInfo(drawingContext, this.sightCenterX, this.sightCenterY, this.sightIntersects);
        }
    }
            ...
            for (let i = 0; i < players.length; i++) {
                ...
                if (player && !player.isOtherPlayer()) {
                    ...
                    if (segments && segments.length > 0) {
                        this.sightIntersects = this.getSightPolygon(player, cameraClass, segments, lightDegreeRange);
                        this.sightCenterX = x;
                        this.sightCenterY = y;
                    ...
                    this.sightDrawingContext.beginPath();
                    this.sightDrawingContext.moveTo(x, y);
                    this.sightDrawingContext.arc(x, y, range, (-lightDegreeRange + player.getDirection()) * Math.PI / 180, (lightDegreeRange + player.getDirection()) * Math.PI / 180, false);
                    ...
                    if (player.getStatus() === 'shoot' && player.getCurrentStatusFrame() == 0) {
                        ...
                        if (shootInfo) {
                            ...
                            bulletLight.addColorStop(0, 'rgba(150, 150, 150,' + darkness + ')');
