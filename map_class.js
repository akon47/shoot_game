class MapClass {
    ...
    while (true) {
      ...
      if (leftTopRight) {
        ...
        result.push({
          ...
          right:
            leftTopRight.left * this.tileWidth +
            ((leftTopRight.right - leftTopRight.left) * this.tileWidth +
              this.tileWidth),
          bottom:
            leftTopRight.top * this.tileHeight +
            ((bottom - leftTopRight.top) * this.tileHeight + this.tileHeight),
        });
      } else {
        break;
      }
  ...
  drawMap(drawingContext, cameraClass, screenWidth, screenHeight) {
    if (drawingContext && cameraClass) {
      ...
      const drawOffsetY =
        cameraClass.getViewboxHeight() / 2 - cameraClass.getViewboxCenterY();
  ...
  getSegments(cameraClass, range) {
    ...
    if (cameraClass) {
      for (let i = 0; i < this.segments.length; i++) {
        var segment = {
          a: {
            x: this.segments[i].a.x - cameraClass.getViewboxLeft(),
            y: this.segments[i].a.y - cameraClass.getViewboxTop(),
          },
          b: {
            x: this.segments[i].b.x - cameraClass.getViewboxLeft(),
            y: this.segments[i].b.y - cameraClass.getViewboxTop(),
          },
        };
    ...
    if (rangeBox) {
      for (let j = 0; j < hitBoxSegments.length; j++) {
        if (
          (rangeBox.left <= hitBoxSegments[j].a.x &&
            rangeBox.right >= hitBoxSegments[j].a.x &&
            rangeBox.top <= hitBoxSegments[j].a.y &&
            rangeBox.bottom >= hitBoxSegments[j].a.y) ||
          (rangeBox.left <= hitBoxSegments[j].b.x &&
            rangeBox.right >= hitBoxSegments[j].b.x &&
            rangeBox.top <= hitBoxSegments[j].b.y &&
            rangeBox.bottom >= hitBoxSegments[j].b.y)
        ) {
